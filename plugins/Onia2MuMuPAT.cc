#include "Onia/dimuonAnalyzer/interface/Onia2MuMuPAT.h"
...
#include "TVector3.h"
#include "Onia/dimuonAnalyzer/interface/OniaVtxReProducer.h"
    ...
    for (View<pat::Muon>::const_iterator it2 = it + 1; it2 != itend; ++it2) {
      ...
      if (it->track().isNonnull() && it2->track().isNonnull()) {
        ...
        if (!myVertex.isValid()) continue;
          ...
          myCand.addUserFloat("cosAlphaBS", cosAlpha);
...
void Onia2MuMuPAT::endJob() {
  std::cout << "###########################" << std::endl;
  std::cout << "Onia2MuMuPAT report:" << std::endl;
  std::cout << "###########################" << std::endl;
